...
export function generateStaticParams() {
  return resourcesByCategory("guide").map((r) => ({ slug: r.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const r = getResource(params.slug);
  if (!r || r.category !== "guide") return { title: "Guide not found" };
  const url = `${siteConfig.url}/resources/guides/${r.slug}`;
  return {
    title: r.seoTitle,
...
export default function GuidePage({ params }: { params: { slug: string } }) {
  const r = getResource(params.slug);
  if (!r || r.category !== "guide") notFound();
  const url = `${siteConfig.url}/resources/guides/${r.slug}`;
  return (
    <>
      <JsonLd
        data={[
          ...
          breadcrumbSchema([
            ...
            { name: "Guides", url: `${siteConfig.url}/resources/guides` },
            { name: r.title, url },
          ]),
        ]}
      />
