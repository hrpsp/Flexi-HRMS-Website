...
export function ResourceCard({ r, hrefBase = "/resources/blog" }: { r: Resource; hrefBase?: string }) {
  ...
  return (
    <Link
      href={href}
      className="group rounded-2xl border border-border bg-card hover:border-brand-dark/30 hover:shadow-md transition-all duration-fast ease-flexi-snap p-6 flex flex-col"
    >
      ...
      <h3 className="text-lg font-semibold text-neutral-text group-hover:text-brand-dark text-pretty">{r.title}</h3>
      <p className="mt-2 text-sm text-neutral-gray line-clamp-3 text-pretty">{r.description}</p>
      <div className="mt-5 pt-4 border-t border-border flex items-center justify-between text-xs text-neutral-gray">
